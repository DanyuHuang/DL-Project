...
def im_detect_all(model, im, support_data, support_box, support_cls, query_path, box_proposals=None, timers=None):
    ...
    scores, boxes, im_scale, blob_conv = im_detect_bbox(
        model, im, support_data, support_box, support_cls, query_path, cfg.TEST.SCALE, cfg.TEST.MAX_SIZE, box_proposals)
    all_cls = scores[:, -1]
    all_scores = scores[:, :-1]
    ...
    for cls_id, cls in enumerate(support_cls):
        ...
        scores_now, boxes_now, cls_boxes_now, nms_cls = box_results_with_nms_and_limit(all_scores[cls_inds], boxes[cls_inds], all_cls[cls_inds])
        if cls_id == 0:
...
def im_detect_bbox(model, im, support_data, support_box, support_cls, query_path, target_scale, target_max_size, boxes=None):
    ...
    # Construct new inputs to input support data and boxes 
    box_dict = {}
    
    inputs['support_data'] = [torch.from_numpy(support_data[np.newaxis,:,:,:])]
    '''
    inputs['support_data'] = torch.from_numpy(support_data[np.newaxis,:,:,:]) # more support
    '''
    box_dict['support_boxes'] = support_box
    box_dict['query_path'] = query_path
    box_dict['support_cls'] = support_cls
    inputs['roidb'] = [[box_dict]]
    ...
    if cfg.TEST.BBOX_REG:
        ...
        boxes = np.tile(boxes, (int(box_deltas.shape[0] / boxes.shape[0]), 1))
...
def box_results_with_nms_and_limit(scores, boxes, all_cls):  # NOTE: support single-batch
    ...
    for j in range(1, num_classes):
        ...
        cls_j = all_cls[inds, 0]
        boxes_j = boxes[inds, j * 4:(j + 1) * 4]
        dets_j = np.hstack((boxes_j, scores_j[:, np.newaxis])).astype(np.float32, copy=False)
        dets_j = np.hstack((dets_j, cls_j[:, np.newaxis])).astype(np.float32, copy=False)
    ...
    boxes = im_results[:, :-2]
    scores = im_results[:, -2]
    nms_cls = im_results[:, -1]
    return scores, boxes, cls_boxes, nms_cls
