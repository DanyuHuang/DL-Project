...
class SQCorrelationModule(nn.Module):
    ...
    def forward(self, input_q, input_s):
        ...
        cor_qs = model_q * model_s #逐个对应相乘 #a与b做*乘法，原则是如果a与b的size不同，则以某种方式将a或b进行复制，使得复制后的a和b的size相同，然后再将a和b做element-wise的乘法。

        cor_qs = cor_qs.unsqueeze(-1).unsqueeze(-1) # increase 2 dimension to 4
        #在expand中的-1表示取当前所在维度的尺寸，也就是表示当前维度不变
        output_q = input_q * cor_qs.expand(-1, -1, input_q.shape[-2], input_q.shape[-1]) #shape[-1] 列数 shape[-2] 行数
        output_s = input_s * cor_qs.expand(-1, -1, input_s.shape[-2], input_s.shape[-1])

        output_q = torch.mean(output_q, 0).unsqueeze(0) #torch.mean(tensor, dim=0) dim 表示的是 沿着 dim 维度进行 求平均值。降低了一维，所以unsqueeze增加一维
